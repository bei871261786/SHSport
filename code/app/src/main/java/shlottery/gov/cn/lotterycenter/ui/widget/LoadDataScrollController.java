...
public class LoadDataScrollController extends RecyclerView.OnScrollListener implements SwipeRefreshLayout.OnRefreshListener {
    ...
    public void onScrollStateChanged(RecyclerView recyclerView, int newState) {
        ...
        LogUtils.i("LoadDataScrollControl onScrollStateChanged:" + visibleCount + "::" + newState + ":::" + mLastVisibleItemPosition + ":::" + (totalCount - 1) + "::::" + !isLoadData);
        if (visibleCount < totalCount //可视条目数量小于条目总数时才允许加载更多。。。
                && newState == RecyclerView.SCROLL_STATE_IDLE
                && mLastVisibleItemPosition >= totalCount - 1
                && !isLoadData) {
            //可以加载数据
            LogUtils.i("LoadDataScrollControl loadMore  can:");
            if (mListener != null) {
                isLoadData = true;
                LogUtils.i("LoadDataScrollControl loadMore:");
                mListener.loadMore();
            }
        }
