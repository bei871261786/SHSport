...
import shlottery.gov.cn.lotterycenter.ui.calculatebouns.adapter.Sf14CalculateLsvAdapter;
 ...
 * 作    者：booob
 * 版    本：1.0
 * 创建日期：2016-12-03-0003 10:50
 * 描    述：
 * 修订历史：
 * ================================================
 */

public class CalSf9Fragment extends CalculateBaseFragment {
    ...
    public void updatestickyUi(CalculateEventBus messageEvent) {
        ...
        if (issueBonusBean != null) {
            for (int i = 0; i < issueBonusBean.getData().getList().size(); i++) {
                if (issueBonusBean.getData().getList().get(i).getLotId().equals("sfc")) {
                    ...
                    sfcBall13Tv.setText(arr[12]);
                    sfcBall14Tv.setText(arr[13]);
    ...
    public void updateUi(CalculateEventBus messageEvent) {
        ...
        if (issueBonusSoccerDetailBean != null) {
            if (issueBonusSoccerDetailBean.getData().getLotId().equals("sfc")) {
                ...
                sf14CalculateLsvAdapter = new Sf14CalculateLsvAdapter(MatchListBeans, getActivity(), mHandler, Configs.ACTIVITYTYPE_R9);
    ...
    public void initLotteryData() {
        ...
        sfcDingdanBean.setIssueNo(issueNo);
        sfcDingdanBean.setLotId("sf9");
        numLotOrderBean.setLotId("sf9");

        numLotOrderBean.setSfcDingdanBean(sfcDingdanBean);
        numLotOrderBean.setIssueNo(issueNo);
        numLotOrderBean.setChuanshu(count);
        numLotOrderBean.setMultiple(calculateMultipleEdit.getText().toString());
        if (count == 9) {
            numLotOrderBean.setCount(zhushu);
            if (count == 1) {
                numLotOrderBean.setGuoguanType("d");//单式
            } else {
                numLotOrderBean.setGuoguanType("f");//复式
            }
        } else {
            numLotOrderBean.setCount(0);
            numLotOrderBean.setGuoguanType("dt");//胆拖
        }
    ...
    public void onShowMessageEvent(SfcEventbus messageEvent) {
        DanNum = 0;
        count = 0;
        for (int i = 0; i < sf14CalculateLsvAdapter.getmSf14Beans().size(); i++) {
            if (!(sf14CalculateLsvAdapter.getmSf14Beans().get(i).getCheckedHashMap().get(0)) && (!sf14CalculateLsvAdapter.getmSf14Beans().get(i).getCheckedHashMap().get(1)) && (!sf14CalculateLsvAdapter.getmSf14Beans().get(i).getCheckedHashMap().get(2))) {
                sf14CalculateLsvAdapter.getmSf14Beans().get(i).setChecked(false);
                sf14CalculateLsvAdapter.getmSf14Beans().get(i).setCanChecked(false);
            } else {
                sf14CalculateLsvAdapter.getmSf14Beans().get(i).setCanChecked(true);
                count++;
            }
        }
//        count = CacluteZhushu(sf14CalculateLsvAdapter.getmSf14Beans());

        sf14CalculateLsvAdapter.setCountNum(count);
        sf14CalculateLsvAdapter.notifyDataSetChanged();
        AsyncTaskUtils.doAsync(new IDataCallBack<Integer>() {
            ...
            public Integer onTasking(Void... params) {
                int money = CacluteZhushu(sf14CalculateLsvAdapter.getmSf14Beans());
                return money;
            }
            ...
            public void onTaskAfter(Integer result) {
                if (result > 0) {
                    if (count == 9) {
                        ...
                        calculateCountTv.setText(result + "注");
                        calculateMoneyTv.setText(result * 2 * value + "元");
                        TextUtils.setStrColor(calculateCountTv, result + "注", result + "", getActivity().getResources().getColor(R.color.select_red));
                        TextUtils.setStrColor(calculateMoneyTv, result * 2 * value + "元", result * 2 * value + "", getActivity().getResources().getColor(R.color.select_red));

                    } else {
                        calculateCountTv.setText(0 + "注");
                        calculateMoneyTv.setText(0  + "元");
                        TextUtils.setStrColor(calculateCountTv, 0+ "注", 0+ "", getActivity().getResources().getColor(R.color.select_red));
                        TextUtils.setStrColor(calculateMoneyTv, 0+ "元", 0 + "", getActivity().getResources().getColor(R.color.select_red));
...
//            CalculateStage. getRenxuan9Count1(unDanNum, danSelectNum);
    ...
    private void loadData() {
        OkGo.get(UrlManager.getBonusDetail(getActivity(), "sfc", issueNo)).cacheMode(CacheMode.NO_CACHE).connTimeOut(5000).execute(new StringCallback() {
            @Override
