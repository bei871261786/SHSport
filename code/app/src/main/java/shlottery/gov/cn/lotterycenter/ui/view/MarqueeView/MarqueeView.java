package shlottery.gov.cn.lotterycenter.ui.view.MarqueeView;
 ...
 * 作    者：booob
 * 版    本：1.0
 * 创建日期：2016-10-10-0010 10:24
 * 描    述：四种方向的滚动公告;项目地址 https://github.com/58252078/MarqueeView
 * 修订历史：
 * ================================================
 */
...
public class MarqueeView extends ViewGroup {
            ...
            switch (currentOrientation) {
                case ORIENTATION_TO_TOP:
                    if (currentPosition >= itemCount - 1) {
                        ...
                        LogUtils.i("MarqueeView computeScroll toTop currentPosition:" + currentPosition + "::itemCount:" + itemCount);
                        LogUtils.i("MarqueeView computeScroll toTop  currentPosition * scrollDistance:" + currentPosition * scrollDistance);
                        currentPosition = 0;
                    }
                    break;
    ...
    private void smoothScroll(int distance) {
        LogUtils.i("MarqueeView smoothScroll Y1:" + scroller.getFinalY() + "::Y2:" + distance);
        if (currentOrientation == ORIENTATION_TO_BOTTOM || currentOrientation == ORIENTATION_TO_TOP) {
    ...
    private void fastScroll(int distance) {
        LogUtils.i("MarqueeView fastScroll Y1:" + scroller.getFinalY() + "::Y2:" + distance);
        if (currentOrientation == ORIENTATION_TO_BOTTOM || currentOrientation == ORIENTATION_TO_TOP) {
    ...
    private class SwitchTimerTask extends TimerTask {
        ...
        @Override
        public void run() {
            LogUtils.i("MarqueeView SwitchTimerTask");
            switch (currentOrientation) {
                case ORIENTATION_TO_TOP:
                    currentPosition++;
                    if (currentPosition >= itemCount) {
                        LogUtils.i("MarqueeView SwitchTimerTask toTop  currentPosition:" + currentPosition);
                        LogUtils.i("MarqueeView SwitchTimerTask toTop  currentPosition * scrollDistance:" + (currentPosition - 1) * scrollDistance);
                        fastScroll(-(currentPosition - 1) * scrollDistance);
                        currentPosition = 0;
                    } else {
