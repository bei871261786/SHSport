...
import shlottery.gov.cn.lotterycenter.ui.calculatebouns.adapter.CalculateJqcLsvAdapter;
...
public class CalaculateJqcFragment extends CalculateBaseFragment {
    ...
    private CalculateJqcLsvAdapter sf14CalculateLsvAdapter;
    ...
    public void updatestickyUi(CalculateEventBus messageEvent) {
        ...
        if (issueBonusBean != null) {
            for (int i = 0; i < issueBonusBean.getData().getList().size(); i++) {
                if (issueBonusBean.getData().getList().get(i).getLotId().equals("jqc")) {
                    ...
                    sfcBall9Tv.setVisibility(View.GONE);
                    sfcBall10Tv.setVisibility(View.GONE);
                    sfcBall11Tv.setVisibility(View.GONE);
                    sfcBall12Tv.setVisibility(View.GONE);
    ...
    public void updateUi(CalculateEventBus messageEvent) {
        ...
        if (issueBonusSoccerDetailBean != null) {
            if (issueBonusSoccerDetailBean.getData().getLotId().equals("jqc")) {
                ...
                sf14CalculateLsvAdapter = new CalculateJqcLsvAdapter(MatchListBeans, getActivity(), mHandler);
    ...
    public void initLotteryData() {
        ...
        sfcDingdanBean.setLotId("jqc");
        numLotOrderBean.setLotId("jqc");
    ...
    private void loadData() {
        OkGo.get(UrlManager.getBonusDetail(getActivity(), "jqc", issueNo)).cacheMode(CacheMode.NO_CACHE).connTimeOut(5000).execute(new StringCallback() {
            @Override
