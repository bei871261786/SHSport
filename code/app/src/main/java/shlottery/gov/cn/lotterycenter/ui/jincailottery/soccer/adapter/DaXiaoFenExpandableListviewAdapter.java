 ...
 */
public class DaXiaoFenExpandableListviewAdapter extends BaseExpandableListAdapter {
    ...
    public DaXiaoFenExpandableListviewAdapter(List<List<MatchsBean>> mDatas, Handler handler) {
    ...
    @Override
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertViews, ViewGroup parent) {
        ChildHolder childHolder;
        if (convertViews == null) {
            convertViews = mInflater.inflate(R.layout.dx_match_expandablelistview_child_item, null);
        ...
        childHolder.match_zhu_spf.setText("大分" + " " + mDatas.get(groupPosition).get(childPosition).getSps().get(0));
        double hand = mDatas.get(groupPosition).get(childPosition).getHand();
        LogUtils.i("Shengfencha hand:"+hand);
        ...
        childHolder.match_fu_spf.setText("小分" + " " + mDatas.get(groupPosition).get(childPosition).getSps().get(1));
                    ...
                    for (int i = 0; i < mDatas.size(); i++) {
                        for (int j = 0; j < mDatas.get(i).size(); j++) {
                            if (i == groupPosition && j == childPosition) {
                                ...
                                finalChildHolder.match_zhu_spf.setTextColor(context.getResources().getColor(R.color.fb_color_text_matchname));
                            }
                        }
                    }
                }
...
//                                mDatas.get(i).get(j).setSpsLostChecked(false);
                                ...
                                finalChildHolder2.match_fu_spf.setTextColor(context.getResources().getColor(R.color.fb_color_text_matchname));
                            }
                        }
        ...
        try {
            LogUtils.i("adapter spf:" + mDatas.get(groupPosition).get(childPosition).getHostName() + "::" + JcDataUtils.getSpsState(mDatas.get(groupPosition).get(childPosition), 0) + "::" + JcDataUtils.getSpsState(mDatas.get(groupPosition).get(childPosition), 1) + "::" + JcDataUtils.getSpsState(mDatas.get(groupPosition).get(childPosition), 2));
