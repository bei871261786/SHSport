 ...
 */
public class DingDanSpfListViewAdapter extends DingdanBaseAdapter {
    ...
    public DingDanSpfListViewAdapter(Context context, List<MatchsBean> mDatas, Handler handler, boolean isDanguan) {
        super(context);
    ...
    public View getView(final int position, View convertView, ViewGroup parent) {
        Holder holder = null;
        if (convertView == null) {
            convertView = mInflater.inflate(R.layout.football_base_dingdanitem_2, null);
            holder = new Holder();
            ...
            holder.match_ping_spf = (TextView) convertView
                    .findViewById(R.id.dingdan_ping_spf);
            ...
            holder.spt_cbx_d = (CheckBox) convertView.findViewById(R.id.dingdan_cbx_d);
        ...
        holder.match_zhu_spf.setText("主胜" + mDatas.get(position).getSps().get(0));
        holder.match_ping_spf.setText("平" + mDatas.get(position).getSps().get(1));
        holder.match_ping_name.setText("VS");
        holder.match_fu_name.setText(mDatas.get(position).getVisitName());

        holder.match_fu_spf.setText("主负" + mDatas.get(position).getSps().get(1));
        ...
        final Holder finalChildHolder1 = holder;
        holder.spt_cbx_d.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                handler.sendEmptyMessage(1);
            }
        });
        holder.spt_cbx_d.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            @Override
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    for (int i = 0; i < mDatas.size(); i++) {
                    ...
                    for (int i = 0; i < mDatas.size(); i++) {
                        if (position == i) {
                            ...
                            finalChildHolder1.match_ping_spf.setTextColor(Color.parseColor("#ff787878"));
        ...
        holder.spt_cbx_l.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                if (isChecked) {
                    for (int i = 0; i < mDatas.size(); i++) {
                        if (position == i) {
                            JcDataUtils.setSpsState(mDatas.get(i), 2, true);
                    ...
                    for (int i = 0; i < mDatas.size(); i++) {
                        if (position == i) {
                            JcDataUtils.setSpsState(mDatas.get(i), 2, false);
        ...
        holder.spt_cbx_d.setChecked(JcDataUtils.getSpsState(mDatas.get(position), 1));
        holder.spt_cbx_l.setChecked(JcDataUtils.getSpsState(mDatas.get(position), 2));
        if (!mDatas.get(position).getSpsStateMap().get(0) && !mDatas.get(position).getSpsStateMap().get(1) && !mDatas.get(position).getSpsStateMap().get(2)) {
            if (mDatas.get(position).isDan()) {
                mDatas.get(position).setDan(false);
                mDanCunt--;
            }
