 ...
 */
public class ShengPingFuExpandableListviewAdapter extends BaseExpandableListAdapter {
    ...
    public ShengPingFuExpandableListviewAdapter(List<List<MatchsBean>> mDatas, Handler handler) {
    ...
    public View getChildView(final int groupPosition, final int childPosition, boolean isLastChild, View convertViews, ViewGroup parent) {
        ...
        if (convertViews == null) {
            convertViews = mInflater.inflate(R.layout.spf_match_expandablelistview_child_item, null);
            ...
            childHolder.match_ping_spf = (TextView) convertViews
            ...
            childHolder.spt_cbx_d = (CheckBox) convertViews.findViewById(R.id.spt_cbx_d);
        ...
        childHolder.spt_cbx_d.setOnCheckedChangeListener(new CompoundButton.OnCheckedChangeListener() {
            ...
            public void onCheckedChanged(CompoundButton buttonView, boolean isChecked) {
                ...
                LogUtils.i("onEventMainThread cb d");
                Message message = new Message();
                Bundle bundle = new Bundle();
                message.setData(bundle);
                handler.sendMessage(message);
                if (isChecked) {
                    for (int i = 0; i < mDatas.size(); i++) {
                        for (int j = 0; j < mDatas.get(i).size(); j++) {
                            if (i == groupPosition && j == childPosition) {
//                                mDatas.get(i).get(j).setSpsPingChecked(true);
                    ...
                    for (int i = 0; i < mDatas.size(); i++) {
                        for (int j = 0; j < mDatas.get(i).size(); j++) {
                            if (i == groupPosition && j == childPosition) {
//                                mDatas.get(i).get(j).setSpsPingChecked(false);
